...
import { CheckCircle, XCircle, ChevronDown, Users, Calendar } from 'lucide-react'
...
function fallbackStudents(classId) {
...
function getStudents(classId) {
  return STUDENTS_BY_CLASS[classId] || fallbackStudents(classId)
}
...
export default function AttendanceCanvas({ context }) {
  ...
  const [selectedClass, setSelectedClass] = useState(context.classId || '6-B')
          ...
          <div className="w-full bg-danger-light rounded-2xl p-3 mb-5 text-left">
            ...
            <div className="flex flex-wrap gap-1.5">
              {students.filter(s => !attendance[s.id]).map(s => (
                <span key={s.id} className="px-2 py-1 bg-white rounded-xl text-[11px] font-bold text-danger-700 border border-[#FFCDD2]">
                  {s.name}
                </span>
              ))}
            </div>
